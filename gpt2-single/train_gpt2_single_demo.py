...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # 初始化参数
        self.apply(self._init_weights) # 把 self._init_weights 这个函数，应用到当前模型 self 以及它内部所有的子模块上
    ...
    def forward(self, idx, targets=None):
        ...
        x = tok_emb + pos_emb # 广播机制，x 形状为 (B, T, n_embd)
    ...
    def from_pretrained(cls, model_type):
        ...
        sd = model.state_dict()
        sd_keys = sd.keys()
        sd_keys = [k for k in sd_keys if not k.endswith(".attn.bias")]  # 丢弃这个 mask/buffer，它不是参数
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device_type):
        ...
        param_dict = {pn: p for pn, p in self.named_parameters()}  # self.named_parameters()：拿到模型中所有参数及其名字
...
def setup_device():
    # 尝试自动检测设备
    # device = xm.xla_device() # google tpu v5e1
    device = "cpu"
    if torch.cuda.is_available():
        device = "cuda"
    elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available(): # MAC Book
        device = "mps"
...
def get_lr(it, max_lr, min_lr, warmup_steps, max_steps):
    ...
    # 当 decay_ratio = 0 时，cos(0)=1，所以 coeff=1
    # 当 decay_ratio = 0.5 时，cos(pi/2)=0，所以 coeff=0.5
    # 当 decay_ratio = 1 时，cos(pi)=-1，所以 coeff=0
...
def train(model, train_loader, optimizer, device, grad_accum_steps, max_lr, min_lr, warmup_steps, max_steps):
    for step in range(max_steps):
        ...
        for micro_step in range(grad_accum_steps):
            ...
            # 使用 Ampere GPU 的 Bfloat16 格式
            # with torch.autocast(device_type=device, dtype=torch.float16):
            #     logits, loss = model(x, y)

            # 梯度累积会把每次 backward 的梯度相加；
            # 由于损失函数一般reduction="mean", 为了让累计后的梯度等价于“大 batch 上的平均梯度”，
            # 这里必须把每个 micro-batch 的 loss 除以 grad_accum_steps。
            loss = loss / grad_accum_steps
            loss_accum += loss.item()  # 记录原始 loss 用于日志
            loss.backward()
            print(f"step: {step}| micro_step: {micro_step}")

        norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)  # 计算所有参数梯度的全局范数（平方和开根号）. 如果大于 1 ，就把所有梯度等比例缩小. 防止梯度过大导致训练不稳定
        lr = get_lr(step, max_lr=max_lr, min_lr=min_lr, warmup_steps=warmup_steps, max_steps=max_steps)
        for param_group in optimizer.param_groups:
            # Pytorch 允许遍历优化器中的所有参数组，并修改它们的学习率
            param_group["lr"] = lr

        optimizer.step()
        # torch.cuda.synchronize() # 等待 GPU 完成当前迭代的所有计算
        t1 = time.time()
        dt = (t1 - t0) * 1000  # 毫秒
        tokens_processed = train_loader.B * train_loader.T * grad_accum_steps
        tokens_per_sec = tokens_processed / (t1 - t0)
        print(
            f"step: {step} | loss: {loss_accum:.6f} | lr: {lr:.4e} | norm : {norm:.4f} | "
            f"dt: {dt:.2f}ms | tok/sec: {tokens_per_sec:.2f}"
        )


def generate_text(model, device, num_return_sequences=5, max_length=30):
    # prefix tokens
    model.eval()
...
def main():
    ...
    input_path = os.path.join(os.path.dirname(__file__), "input.txt") # 获取绝对路径
    train_loader = DataLoaderLite(B=B, T=T, data_path=input_path)
    ...
    generate_text(model=model, device=device, num_return_sequences=5, max_length=30)
